...
//    val sequenceComponentAllocator = mock[SequenceComponentAllocator]
//    val sequenceComponentUtil1 = new SequenceComponentUtil(locationServiceUtil, sequenceComponentAllocator)
//    val sequencerUtil1 = new SequencerUtil(locationServiceUtil, sequenceComponentUtil1)
//
//    val smBehavior = new SequenceManagerBehavior(
//      config,
//      locationServiceUtil,
//      agentUtil,
//      sequencerUtil,
//      sequenceComponentUtil
//    )
//    lazy val smRef: ActorRef[SequenceManagerMsg] = actorSystem.systemActorOf(smBehavior.setup, "sm_test_actor")
    ...
    "be able to handle next messages if the previous Provision call times-out due to downstream error | ESW-473" in {
      ...
      when(agentUtil.provision(provisionConfig)).thenReturn(failedFuture(exceptionReason, 0.second))
    ...
    "be able to handle next messages if the previous Configure call times-out due to downstream error | ESW-473" in {
      ...
      when(sequencerUtil.startSequencers(darkNight, darkNightSequencers))
        .thenReturn(failedFuture(exceptionReason, delay = 0.second))
    ...
    "be able to handle next messages if the previous RestartSequencer call times-out due to downstream error | ESW-473" in {
      ...
      when(sequencerUtil.restartSequencer(ESW, darkNight)).thenReturn(failedFuture(exceptionReason, 1500.milliseconds))
    ...
    "be able to handle next messages if the previous ShutdownSequenceComponent call times-out due to downstream error | ESW-473" in {
      ...
      when(sequenceComponentUtil.shutdownSequenceComponent(prefix)).thenReturn(failedFuture(exceptionReason, 1500.milliseconds))
